...
writeFileSync(path.join(ROOT,'data/weekly_campaign_2026-09-14.csv'),csv);
const archive=path.join(ROOT,'output/campaign_2026-09-14');mkdirSync(archive,{recursive:true});
writeFileSync(path.join(archive,'approval.json'),JSON.stringify(approval,null,2)+'\n');
copyFileSync(path.join(PREVIEW,'seo-validation.json'),path.join(archive,'seo-validation.json'));
copyFileSync(path.join(PREVIEW,'legendas.md'),path.join(archive,'legendas.md'));
copyFileSync(path.join(PREVIEW,'previa-semana-14-a-20-setembro.jpg'),path.join(ROOT,'output/preview_week_5_styles_2026-09-14_to_2026-09-20.jpg'));
console.log('Staged 35 approved pins with exact SEO metadata, 42 products, 2026-09-14 through 2026-09-20.');
